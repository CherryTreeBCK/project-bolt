...
You are a classification agent. Your task is to complete the "category" and "priority" columns for each account in the provided CSV. 
The data includes: username, full name, ID, follower count, following count, posts count, is_verified, is_private, biography, external URL, and two blank columns: category and priority. 

CATEGORY INSTRUCTIONS: 
Choose exactly one of these 15 categories: 1. real estate 2. dental 3. medspa 4. restaurant 5. home services 6. health 7. exercise 8. beauty 9. fashion 10. coaching 11. art 12. photography 13. music 14. education 15. other business Choose **"other business"** only if none of the first 14 categories apply, but at least **two** of the following are true: - The bio mentions a business or professional role (e.g., owner, founder, coach, etc.) - The bio advertises a service, product, or brand - An external URL is present and likely business-related Leave the category **blank** only if the account is clearly personal (no signs of business, no business-related bio or URL). 

PRIORITY INSTRUCTIONS: 
Priority must be a number from 5 (highest) to 1 (lowest), calculated with the following logic: • Start with priority = 3 • Add 1 if follower_count > 10,000 • Add 1 if is_verified is true • Add 1 if external_url is present (not blank) • Subtract 1 if is_private is true • Subtract 1 if the account is clearly personal (no signs of business, no business-related bio or URL) • Subtract 1 if posts_count < 10
...
ADDITIONAL INSTURCTIONS (FOLLOW STRICTLY):
---

${csvSubset}
`;

async function classifyFollowers(settingsOrCallback = {}, progressCallback) {
	let settings = {};
	if (typeof settingsOrCallback === 'function') {
		progressCallback = settingsOrCallback;
		settings = {};
	} else {
	...
	const minFollowers = Number.isFinite(Number(settings.minFollowers)) ? Number(settings.minFollowers) : 0;
	const extraInstructions = typeof settings.aiPromptAddition === 'string' ? settings.aiPromptAddition : '';

	let query = supabase.from('followers_duplicate_new').select('*');
	...
	for (let start = 0; start < total; start += BATCH_SIZE) {
		...
		try {
			const completion = await openai.chat.completions.create({
				...
				messages: [{
						role: 'system',
						content: 'You are a helpful assistant that processes CSV data.'
					},
					{
						role: 'user',
						content: prompt
